...
func RegisterProcessAPI(mux *http.ServeMux, absPath string, secret []byte) {
	middleware := RequireJWT(secret)

	mux.Handle("GET /api/process/status", middleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		handleStatusGateway(w, r, absPath)
	})))
	mux.Handle("POST /api/process/start", middleware(http.HandlerFunc(handleStartGateway)))
	mux.Handle("POST /api/process/stop", middleware(http.HandlerFunc(handleStopGateway)))
}
